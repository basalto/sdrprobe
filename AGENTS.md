...
- `make` — with no target, prints every target and what it is for. The list
  is generated by `scripts/make_help.py` from the `#:` line above each rule,
  and a `check-*` rule with no such line is described by its own suite's
  `check_report()` sentence, so the two cannot drift apart.
  `check-make-help` fails a target added without either.
- `make all` — builds `sdrprobe` via pkg-config; requires librtlsdr and
  raylib development headers installed.
