...
},
{
    ...
    "200 kHz carrier is a fifth of a pixel wide. + and - zoom, Left and Right "
    "walk the window, 0 goes back to the whole sweep, and the mouse wheel "
    "zooms while the pointer is over the chart. Zooming re-draws the same "
    "measurements larger rather than re-sweeping, so it costs nothing and "
    "changes nothing; the level axis follows what is on screen, so a quiet "
    "band beside a loud one is not left flat on the floor. Zooming in keeps "
    "the selected candidate in view, and selecting one the window has scrolled "
    "past brings the window to it.\n"
    "\n"
