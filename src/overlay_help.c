...
static const struct help_page help_pages[HELP_TOPIC_COUNT] = {
{
    ...
    "Decode is where bits become messages -- Mode S / ADS-B frames, GSM "
    "synchronisation bursts, and LTE cell identities. The LTE screen is the "
    ...
    "Up/Down   zoom the frequency window in and out, on a chart that has "
    "one; the vertical scale on every other chart\n"
    "Left/Right   pan the frequency window; past the edge of what is being "
    "received, it retunes\n"
    "0   back to the whole received span\n"
    ...
    "Esc   back out of one thing: a menu, then a field being typed into, "
    "then a frequency zoom, then the screen. From the Scope tab with none of "
    "those open, it quits\n"
    "\n"
...
},
{
    ...
    "\n"
    "A candidate marked * looks like the receiver rather than the band. Its "
    "frequency is a multiple of 14.4 MHz -- half this dongle's reference clock, "
    "whose harmonics reach the tuner -- or it sits where the receiver's own "
    "DC offset lands and the DC-spike filter is off. A sweep of 470-690 MHz "
    "turns up a dozen of them standing 20 dB above the floor, inside the "
    "television allocation, looking exactly like signals. They are marked and "
    "never removed: a real transmitter can sit on a harmonic.\n"
    "\n"
    "Unplugging the antenna is the test, but read it carefully. Three of that "
    "dozen -- 489.6, 547.2 and 604.8 MHz -- stay exactly where they were, "
    "within half a decibel: those are made and heard entirely inside the "
    "receiver. The other nine vanish with the antenna, because the dongle "
    "radiates its clock and hears itself coming back. Both are the receiver's "
    "doing. So what survives unplugging is certainly an artifact, and what "
    "does not survive is not thereby a signal.\n"
    "\n"
...
},
{
    ...
    "\n"
    "Finding one. The band buttons choose where to look and Scan band walks "
    "it. It has to tune to every channel in turn -- a hundred kilohertz "
    "apart, three hundred of them in band 20 -- because the primary sequence "
    "is found by a correlation in time, which a frequency error of more than "
    "a few kilohertz destroys; there is no sweeping ten channels at once the "
    "way the GSM scan does. So the order is chosen instead: every whole "
    "megahertz first, which is where operators centre their carriers, then "
    "the halves, then the rest. The first pass is done in about thirteen "
    "seconds and usually finds everything, and the scan can be stopped there. "
    "Click a row to tune to it.\n"
    "\n"
    "The funnel above counts blocks, cells, parities that passed, and "
    "messages -- and the gap between the last two is deliberate. Sixteen bits "
    "of parity sound decisive until you count the attempts: four scrambling "
    "offsets against three masks, for each of three antenna-port guesses, is "
    "thirty-six chances a block, so a session that finds a cell nine thousand "
    "times will see one pass by luck. That is the expected number, not a rare "
    "accident. A message is therefore only counted once a second one agrees "
    "about what a cell cannot change between frames: its bandwidth, its "
    "acknowledgement channel and its antenna count.\n"
    "\n"
    "An identity is only listed once it has been seen twice on the same "
    "channel. Three hundred channels is three hundred chances to be wrong, "
    "and a threshold set for looking at one carrier is far too loose for "
    "that: without the repeat, a live scan here listed three cells on "
    "frequencies no carrier is centred on. Noise can clear the gate; it "
    "cannot clear it twice with the same identity out of five hundred and "
    "four. The list is ordered by the strength of the primary correlation, "
    "so the top of it is the one worth clicking and the bottom is where a "
    "confirmed identity is still worth doubting.\n"
    "\n"
    "The frequency offset is worth reading. A dongle's crystal is tens of "
    "parts per million out, which at 800 MHz is tens of kilohertz -- two "
    "whole subcarriers on this hardware. The primary sequence measures a "
    "phase, and a phase wraps every subcarrier, so it can only ever report "
    "the leftover fraction; the whole-subcarrier part is found by trying "
    "them against the secondary sequence, and the panel shows both. Getting "
    "that wrong does not look like an error: the primary sequence still "
    "locks, and everything after it reads the wrong subcarriers and returns "
...
},
{
    ...
    "\n"
    "Two calibration circles at the top right of every screen, one per "
    "reference. Grey means no calibration, green means calibrated, amber means "
    "a measurement is running (it retunes the receiver, so the live view "
    "pauses), and red -- GSM only, being the one with a drift monitor behind "
    "it -- means the correction has moved more than 2 PPM and should be taken "
    "again. The periodic GSM re-check stays off until 'Auto GSM drift check' "
    "is ticked in Settings.\n"
    "\n"
    "Back and Exit are two different things. Back is one step up: from a "
    "measurement to the channel list it was picked from -- the 4G cell list, "
    "or the 2G channel scan, reopened with the results it already has. It is "
    "dim when nothing is being measured, because then the list is already "
    "what is on show. Exit leaves calibration for the survey. Esc follows "
    "Back while there is a step to take, and Exit once there is not.\n"
    "\n"
    "Two references measuring one crystal is the point of having both. GSM "
    "measures an FCCH tone; LTE takes the offset its cell search already "
    "measures, including the whole subcarriers a phase alone cannot see. When "
    "they agree the correction is worth trusting; when they do not, that is "
