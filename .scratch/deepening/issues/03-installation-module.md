...
`receiver-a` -- a real one, so ADR-0018's USB-serial path works.
...
```
Site "site-a" has a -31 ppm correction from before calibrations
named a receiver.
It is not applied. --claim-calibration assigns it to this one
(receiver-a).
```

and after claiming, `known_site 0 site-a` plus
`calibration receiver-a -31 site-a`, with the next run silent
