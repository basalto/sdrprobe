...
static void encode(const uint8_t message[TETRA_SB_MESSAGE_BITS],
    ...
    uint8_t type4[TETRA_SB_SCRAMBLED_BITS];
    unsigned int crc;
    int k, state = 0;

    memcpy(type2, message, TETRA_SB_MESSAGE_BITS);
    crc = tetra_sb_crc(message, TETRA_SB_MESSAGE_BITS);
    for (k = 0; k < TETRA_SB_CRC_BITS; k++)
        type2[TETRA_SB_MESSAGE_BITS + k] =
            (uint8_t)((crc >> (TETRA_SB_CRC_BITS - 1 - k)) & 1u);
    for (k = 76; k < TETRA_SB_TYPE2_BITS; k++)
        type2[k] = 0;
    ...
    for (k = 1; k <= TETRA_SB_SCRAMBLED_BITS; k++) {
        ...
        type4[at - 1] = type3[k - 1];
    }
    tetra_sb_descramble(type4);      /* its own inverse */
    for (k = 0; k < 60; k++)
...
static void test_the_puncturing(void) {
    ...
    check_int("out of range high",
              tetra_rcpc_puncture_index(TETRA_SB_SCRAMBLED_BITS + 1), -1);
}
