...
static void test_a_bare_carrier_says_there_is_nothing_to_decode(void) {
    ...
    check_true("a bare carrier is reported",
               signal_findings_from(&c, NULL, 1.0, 31, 31, 100.0, &f) >= 2);
...
static void test_a_modulated_carrier_says_what_was_not_asked(void) {
    ...
    signal_findings_from(&c, NULL, 1.0, 31, 31, 100.0, &f);
              ...
              mentions(&f, "0%"), 0);
    {
        ...
        signal_findings_from(&big, NULL, 1.0, 31, 31, 100.0, &g);
...
static void test_a_pulse_is_not_a_bare_tone(void) {
     ...
     */
    {
        ...
        signal_findings_from(&c, &pulses, 1.0, 31, 31, 100.0, &g);
    ...
    }
    {
        ...
        busy.occupancy = 0.80;
        signal_findings_from(&c, &busy, 1.0, 31, 31, 100.0, &g);
...
static void test_the_refusals_are_kept(void) {
    ...
    check_int("nothing measured yet says so rather than staying silent",
              signal_findings_from(NULL, NULL, 0.0, 0, 0, 0.0, &f), 1);
...

    {
        struct signal_carrier c = bare();
        c.found = 0;
        signal_findings_from(&c, NULL, 0.0, 0, 0, 0.0, &f);
...
static void test_duty_and_drift_qualify_the_claim(void) {
    ...
    signal_findings_from(&c, NULL, 3.0 / 31.0, 3, 31, 100.0, &f);
    ...
    signal_findings_from(&c, NULL, 1.0, 31, 31, 6400.0, &f);
...
static void test_it_never_overruns(void) {
    ...
    signal_findings_from(&c, NULL, 0.05, 1, 31, 90000.0, &f);
    ...
    check_int("a null destination is refused",
              signal_findings_from(&c, NULL, 1.0, 31, 31, 0.0, NULL), 0);
}
...
static void test_every_branch_fits_the_panel(void) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        for (d = 0; d < 3; d++)
            for (e = 0; e < 4; e++) {
                signal_findings_from(&c, b[e], duties[d], 999, 999, 99900.0,
                                     &f);
                check_msg(f.count <= SIGNAL_FINDING_LINES,
                          "carrier %d duty %d envelope %d overran: %d lines",
                          i, d, e, f.count);
                for (k = 0; k < f.count; k++)
                    check_msg(strlen(f.line[k]) <= SIGNAL_FINDING_FIT,
                              "carrier %d duty %d envelope %d line %d fits: "
                              "%zu of %d -- \"%s\"",
                              i, d, e, k, strlen(f.line[k]),
                              SIGNAL_FINDING_FIT, f.line[k]);
            }
    }
    {
        struct signal_findings f;
        signal_findings_from(NULL, NULL, 0.0, 0, 0, 0.0, &f);
